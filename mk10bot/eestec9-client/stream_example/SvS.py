...
lower = {'Scorpion':(60, 20, 0), 'SubZero':(90, 20, 10)} 
upper = {'Scorpion':(225, 135, 100), 'SubZero':(202, 142, 100)}
...
def example(frame):
    ...
    frame_sz = orig_frame
    frame_scorpion = apply_brightness_contrast(orig_frame, 0 ,35)
    frame_scorpion = cv2.GaussianBlur(frame_scorpion, (11, 11), 0)
    
    #cv2.imshow("ghh", frame_scorpion)

    frame_sz = apply_brightness_contrast(frame_sz, 0 ,35) #85 105
    ...
    if len(cnts) > 0:
        ...
        if radius1 > 5:
            ...
            cv2.circle(orig_frame, (int(x1), int(y1)), int(radius1), colors[key], 2)
            cv2.putText(orig_frame,key, (int(x1-radius1),int(y1-radius1)), cv2.FONT_HERSHEY_SIMPLEX, 0.6,colors[key],2)
    ...
    key = "SubZero"
    kernel = np.ones((9,9),np.uint8)
    frame_sz[int(y1-500):int(y1+500),int(x1-100):int(x1+100)] = [0,0,0]
    orig_frame[int(y1-500):int(y1+500),int(x1-100):int(x1+100)] = [0,0,0]
    ...
    if len(cnts) > 0:
        ...
        if radius2 > 5:
            # draw the circle and centroid on the frame,
            # then update the list of tracked points
            cv2.circle(orig_frame, (int(x2), int(y2 + 0)), int(radius2), colors[key], 2)
            cv2.putText(orig_frame,key, (int(x2-radius2),int(y2-radius2)), cv2.FONT_HERSHEY_SIMPLEX, 0.6,colors[key],2)
